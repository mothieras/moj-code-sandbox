...
class SandboxBenchmarkIT {
    ...
    @Autowired ContainerPoolManager poolManager;
    ...
    private void judgeOnce(PooledContainer c) {
        ...
        executor.exec(c.getContainerId(), 10, null, "javac", "-encoding", "utf-8", "/box/Main.java");
        executor.exec(c.getContainerId(), 10, null, "java", "-cp", "/box", "Main");
        executor.exec(c.getContainerId(), 5, null, "sh", "-c", "rm -rf /box/*");
    }
    ...
    void pooled_vs_per_request_latency() throws Exception {
        ...
        DockerContainerProvider coldProvider =
                new DockerContainerProvider(dockerClient, props, "amazoncorretto:17-alpine");

        // 预热 java 池（触发懒加载 warmUp，排除首轮创建抖动）
        PooledContainer warm = poolManager.borrow("java");
        judgeOnce(warm);
        poolManager.giveBack("java", warm);

        // A) 池化：借→判→还
        long pooledTotal = 0, pooledMax = 0;
        for (int i = 0; i < n; i++) {
            long t0 = System.currentTimeMillis();
            PooledContainer c = poolManager.borrow("java");
            judgeOnce(c);
            poolManager.giveBack("java", c);
        ...
        for (int i = 0; i < n; i++) {
            long t0 = System.currentTimeMillis();
            PooledContainer c = coldProvider.create();
            judgeOnce(c);
            coldProvider.destroy(c);
            long cost = System.currentTimeMillis() - t0;
            coldTotal += cost;
